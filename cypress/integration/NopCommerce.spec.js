describe('NopCommerce test', () => {
    ...
    describe('Product page check', () => {
        ...
        it('Add item to Cart', () => {
            ...
            cy.server().route('POST', 'https://demo.nopcommerce.com/addproducttocart/details/*/*').as('addProductToCart')
            ...
            cy.get('div[data-productid="14"] > .variant-overview > .variant-name').then(($div) => {
    ...
    describe('Check Shopping cart Drop Down properties', () => {
        ...
        it('Assure correct Item Price is displayed', () => {
            cy.get('.price > span').then(($span) => {
                expect(parseInt($span.text().split('$')[1].toString().replace(',', ''))).to.equal(price)
            })
        })
        ...
        it('Assure correct Total item price is displayed', () => {
            cy.get('.totals > strong').then(($strong) => {
                expect(parseInt($strong.text().split('$')[1].toString().replace(',', ''))).to.equal(price * deviceQuantity)
            })
        })
    ...
    describe('Shopping Cart check', () => {
        ...
        it('Assure correct Item Price is displayed', () => {
            cy.get('.product-unit-price').then(($span) => {
                var itemPrice = 0
                itemPrice = parseInt($span.text().split('$')[1].toString().replace(',', ''))
                expect(itemPrice).to.equal(price)
            })
        })

        it('Assure correct Quantity is displayed', () => {
            cy.get('tbody > tr > .quantity > .qty-input').invoke('val').then((quantity) => {
        ...
        it('Assure correct Total item price is displayed', () => {
            ...
            cy.get('.product-subtotal').then(($span) => {
                var total = 0
                total = parseInt($span.text().split('$')[1].toString().replace(',', ''))
